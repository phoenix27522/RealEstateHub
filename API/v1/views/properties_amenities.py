...
@swag_from('documentation/place_amenity/get_places_amenities.yml',
           methods=['GET'])
def get_place_amenities(property_id):
    """
    Retrieves the list of all Amenity objects of a Place
    """
    place = storage.get(Property, property_id)
    ...
    if environ.get('HBNB_TYPE_STORAGE') == "db":
        ...
        amenities = [storage.get(Amenity, amenity_id).to_dict()
                     for amenity_id in property.amenity_ids]
...
@swag_from('documentation/place_amenity/delete_place_amenities.yml',
           methods=['DELETE'])
def delete_place_amenity(property_id, amenity_id):
    """
    ...
    if environ.get('HBNB_TYPE_STORAGE') == "db":
        ...
        if amenity_id not in place.amenity_ids:
            abort(404)
        property.amenity_ids.remove(amenity_id)
...
@swag_from('documentation/place_amenity/post_place_amenities.yml',
           methods=['POST'])
def post_place_amenity(property_id, amenity_id):
    """
